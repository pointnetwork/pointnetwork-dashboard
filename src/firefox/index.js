...
class Firefox {
  ...
  async download(win) {
    ...
    const pacFile = url.pathToFileURL(
      path.join(await helpers.getPNPath(osAndArch), "client", "proxy", "pac.js")
    );
...
`;
  ...
  async isFirefoxRanOnce() {
    ...
    return fs.pathExistsSync(
      path.join(pointPath, INSTALLER_FINISHED_FLAG_PATH)
    );
  }
