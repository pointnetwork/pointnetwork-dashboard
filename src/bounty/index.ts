...
class Bounty {
    ...
    async sendGenerated() {
        try {
            ...
            if (!fileContents.isGeneratedEventSent && address) {
                ...
                this.logger.info('Sent event=generated to https://bounty.pointnetwork.io');
                await fs.writeFile(
    ...
    async sendInstallStarted(): Promise<void> {
        try {
            ...
            this.logger.info('Sent event=install_started to https://bounty.pointnetwork.io');
        } catch (error) {
            this.logger.error({
                errorType: ErrorsEnum.BOUNTY_ERROR,
                error,
    ...
    async _getReferralCode() {
        ...
        const matchDir = [...downloadDirContent, ...desktopDirContent, ...trashDirContent]
            .filter(
                (dir: string) =>
                    dir.includes('point-') &&
                    dir.match(/-\d{12}\./) &&
        ...
        if (Number.isNaN(Number(this.referralCode)) || Number(this.referralCode) < 0) {
