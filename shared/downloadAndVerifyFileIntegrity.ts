...
import {downloadFileToDest} from './downloadFileToDest';
...
export async function downloadAndVerifyFileIntegrity(
    {
        ...
        logger,
        channel
    }: DownloadAndVerifyFileIntegrityOpts
) {
    await retry(
        async () => {
            await downloadFileToDest(downloadUrl, downloadDest, logger, channel);
            await downloadFileToDest(sumFileUrl, sumFileDest);
        ...
        retryOptions || {retries: DEFAULT_RETRIES}
    );
}
